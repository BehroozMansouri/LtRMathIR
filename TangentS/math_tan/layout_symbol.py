...
class LayoutSymbol(MathSymbol):
    ...
    def get_element_children(self):
        ...
        if self.within:
            ...
            while children[0].element:
                children.append(children[0].element)
        ...
        if len(children) < 4 and invisible_matrix(children[1]): # fenced matrix (but omit closing tag, as below)
            ...
            if (len(children) > 2):
                ...
                if len(children) == 3:  # (fence,expr-list,fence) => look for separators
                    ...
                    for atom_num in range(2,len(children)):         # no nested mrow: break when argument is a separator
                        if separates(children[atom_num].tag):
                            ...
                            if separates(expr.tag): # previous element was a separator
                                ...
                                while expr.next:
                                    expr = expr.next
                                expr.next = children[atom_num]
                mnode.tag = 'M!' + children[0].tag + children[0].tag + '1x' + str(num_args) # as if fenced 1xn matrix
            else:
                mnode.tag = 'M!' + children[0].tag + (children[0].tag if len(children)>1 else '')
            return mnode
        ...
        if elem.tag == MathML.math:
            ...
            if len(children) > 0:
                ...
                if (len(children) > 2 and (children[0].tag in '({|∥' or children[0].tag == "&lsqb;")
                    and (children[0].tag in ')}|∥' or children[0].tag == "&rsqb;")):  # bracketed expression: treat as matrix
