...
## simplify math_util extraction by creating simple list of math_util expressions and then grouping them by SLT, rather than by LaTeX
...
class MathExtractor:
    ...
    inner_math = re.compile(".*(<" + math_expr + ")", re.DOTALL)  # rightmost <*:math_util
    ...
    def math_tokens(cls, content):
        ...
        return: embedded math_util expressions
    ...
    @classmethod
    def convert_mathml_slt(cls, mathml):
        pmml = cls.isolate_pmml(mathml)
        return SymbolTree(cls.convert_to_layoutsymbol(pmml))

    @classmethod
    def convert_mathml_opt(cls, mathml):
        cmml = cls.isolate_cmml(mathml)
        return SymbolTree(cls.convert_to_semanticsymbol(cmml))
    ...
    @classmethod
    def parse_from_tex22(cls, tex, file_id=-1, position=[0]):
        """
        Parse expression from Tex string using latexmlmath to convert to presentation markup language
